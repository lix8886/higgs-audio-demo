...
class HiggsAudioTokenizer(nn.Module):
    def __init__(
        ...
        semantic_model_or_path: str = "bosonai/hubert_base",
        ...
        if semantic_techer == "hubert_base":
            self.semantic_model = AutoModel.from_pretrained(
                semantic_model_or_path or "facebook/hubert-base-ls960"
            )
            ...
            self.semantic_model = AutoModel.from_pretrained(
                semantic_model_or_path or "microsoft/wavlm-base-plus"
            )
            self.semantic_sample_rate = 16000
            self.semantic_dim = 768
            self.encoder_semantic_dim = 768
            ...
            self.semantic_model = AutoModel.from_pretrained(
                semantic_model_or_path or "bosonai/hubert_base", trust_remote_code=True
            )
            self.semantic_sample_rate = 16000
            self.semantic_dim = 768
            self.encoder_semantic_dim = 768
        ...
        if isinstance(bins, int):  # RVQ
            ...
            self.quantizer = ResidualFSQ(
                dim=self.quantizer_dim, levels=bins, num_quantizers=n_q
            )
    ...
    def get_regress_target(self, x):
        x = torchaudio.functional.resample(
            x, self.sample_rate, self.semantic_sample_rate
        )
    ...
    def _xcodec_encode(
        ...
        if e_acoustic.shape[2] != e_semantic.shape[2]:
            ...
            e_acoustic = self.encoder(
                F.pad(x[:, 0, :], (pad_size, pad_size)).unsqueeze(0)
            )
...
def load_higgs_audio_tokenizer(tokenizer_name_or_path, semantic_model_or_path, device="cuda"):
    ...
    model = HiggsAudioTokenizer(
        **config,
        semantic_model_or_path=semantic_model_or_path,
        device=device,
    )
