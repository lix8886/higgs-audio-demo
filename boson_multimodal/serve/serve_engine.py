...
class AsyncHiggsAudioStreamer(BaseStreamer):
    ...
    def put(self, value: torch.Tensor):
        ...
        if value.shape[0] > 1 and not self.next_tokens_are_prompt:
            ...
            assert (
                value.shape[0] == self.audio_num_codebooks
            ), "Number of codebooks mismatch"
...
class HiggsAudioServeEngine:
    def __init__(
        ...
        semantic_model_or_path: str = None,
        ...
        logger.info(
            f"Loaded model from {model_name_or_path}, dtype: {self.model.dtype}"
        )
        ...
        self.samples_per_token = int(
            self.audio_tokenizer.sampling_rate // self.audio_tokenizer_tps
        )
        ...
        if self.model.config.audio_dual_ffn_layers:
            cache_config.num_hidden_layers += len(
                self.model.config.audio_dual_ffn_layers
            )
    ...
    def _prepare_inputs(
        ...
        if len(audio_ids_l) > 0:
            audio_ids_start = torch.tensor(
                np.cumsum(
                    np.array([0] + [audio_ids.shape[1] for audio_ids in audio_ids_l])
                ),
                dtype=torch.long,
                device=self.device,
            )[0:-1]
    ...
    def generate(
        ...
        with torch.no_grad():
            inputs = self._prepare_inputs(
                chat_ml_sample, force_audio_gen=force_audio_gen
            )
