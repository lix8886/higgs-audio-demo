...
class HiggsAudioModelClient:
    def __init__(
        ...
        snapshot_download(
            model_path,
            local_dir=f'models/{os.path.basename(model_path)}'
        )
        snapshot_download(
            audio_tokenizer,
            local_dir=f'models/{os.path.basename(audio_tokenizer)}'
        )
        snapshot_download(
            semantic_model_or_path,
            local_dir=f'models/{os.path.basename(semantic_model_or_path)}'
        )
        ...
        self._audio_tokenizer = load_higgs_audio_tokenizer(
            audio_tokenizer, semantic_model_or_path, device=audio_tokenizer_device
        )
    ...
    def create(
        self,
        transcript,  # 文本
        scene_prompt="Audio is recorded from a quiet room.",  # 情景描述
        temperature=1.0,
        top_k=50,
        top_p=0.95,
        ras_win_len=7,
        ras_win_max_num_repeat=2,
        ref_audio=None,  # 参考音频，xxx,yyy
        chunk_method=None,  # 可选值: None, "speaker", "word"
        ...
        messages, audio_ids = self.prepare_generation_context(
        ...
        concat_wv, sr, text_output = self.generate(
            messages=messages,
            audio_ids=audio_ids,
    ...
    def prepare_generation_context(
        ...
        if ref_audio is not None:
            ...
            ref_audio_profile = False
            # 说话者的语音特征，例如：male_en: Male, American accent, modern speaking rate, moderate-pitch, friendly tone, and very clear audio.
            profile_path = f"{self.reference_path}/voice_profile.json"
            with open(profile_path, "r", encoding="utf-8") as f:
                voice_profile = json.load(f)

            # 处理参考音频 + 说话者信息voice profile
            if any([s.startswith("profile:") for s in speaker_info_l]):
                ref_audio_profile = True
                speaker_desc = []
                for spk_id, character_name in enumerate(speaker_info_l):
                    if character_name.startswith("profile:"):
                        character_desc = voice_profile[
                            character_name[len("profile:"):].strip()
                        ]
                        speaker_desc.append(f"SPEAKER{spk_id}: {character_desc}")
                    else:
                        speaker_desc.append(
                            f"SPEAKER{spk_id}: {AUDIO_PLACEHOLDER_TOKEN}"
                        )
                if scene_prompt:
                    system_message = (
            ...
            for spk_id, character_name in enumerate(ref_audio.split(",")):
                if not character_name.startswith("profile:"):
                    prompt_audio_path = os.path.join(
                        f"{self.reference_path}/voice_prompts", f"{character_name}.wav"
                    )
                    prompt_text_path = os.path.join(
                        f"{self.reference_path}/voice_prompts", f"{character_name}.txt"
                    )
